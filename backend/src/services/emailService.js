...
export const sendVerificationEmail = async (toEmail, userName, code, role = 'patient') => {
  const isClinicStaff = role === 'clinic_staff';

  const footerMessage = isClinicStaff
    ? 'After confirming your email, your account will be reviewed by an administrator before you can log in.'
  ...
  await mailer.sendMail({
    ...
    html: `
          <div style="font-family: sans-serif; max-width: 480px; margin: auto;">
            ...
            <p style="color:#6B7280;font-size:14px;">${footerMessage}</p>
          </div>`,
  });
};
